...
JTetromino::JTetromino(IState* stateInstance):Tetromino(stateInstance)
{
	_maxRotations = 4;
	...
	second->push_back( CreateRow( _columns, 1, 1, 1, 0 ) );
	second->push_back( CreateRow( _columns, 0, 0, 0, 0 ) );
	second->push_back( CreateRow( _columns, 0, 0, 0, 0 ) );

	vector<vector<int>*>* third  = new vector<vector<int>*>();
	third->push_back( CreateRow( _columns, 1, 1, 0, 0 ) );
	third->push_back( CreateRow( _columns, 1, 0, 0, 0 ) );
	third->push_back( CreateRow( _columns, 1, 0, 0, 0 ) );
	third->push_back( CreateRow( _columns, 0, 0, 0, 0 ) );

	vector<vector<int>*>* fourth = new vector<vector<int>*>();
	fourth->push_back( CreateRow( _columns, 1, 1, 1, 0 ) );
	fourth->push_back( CreateRow( _columns, 0, 0, 1, 0 ) );
	...
	_spriteURL = "assets/Gameplay/Tetromino/J.png";
}

JTetromino::~JTetromino(void)
{
}
